...
function newGame() {
  ...
  document.body.addEventListener('touchstart', ({ keyCode }) => {
    if (!game.charging) game.handleJumperCharging();
    if (game.progressing) {
      if (keyCode === arrowR) game.handleJumperSpin(1);
      if (keyCode === arrowL) game.handleJumperSpin(-1);
    } else {
  ...
  document.body.addEventListener('touchend', ({ keyCode }) => {
    if (game.charging) game.handleJumperFire();
    if (game.progressing) {
      if (keyCode === arrowD) game.handleJumperDown();
    }
  });
